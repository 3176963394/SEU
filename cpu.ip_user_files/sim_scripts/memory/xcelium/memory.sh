...
#
# Script generated by Vivado on Wed May 29 13:23:00 +0800 2024
...
#
# Usage        : memory.sh
#                memory.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                memory.sh [-reset_run]
#                memory.sh [-reset_log]
#                memory.sh [-help]
#
...
#                script (type 'memory.sh -help' for more information).
#
...
# script info
echo -e "memory.sh - Script generated by export_simulation (Vivado v2023.2 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./memory.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  xmelab $xmelab_opts $design_libs_elab xil_defaultlib.memory xil_defaultlib.glbl
}

# RUN_STEP: <simulate>
simulate()
{
  xmsim $xmsim_opts xil_defaultlib.memory -input simulate.do
}
...
reset_run()
{
  files_to_remove=(xmvlog.log xmvhdl.log xmsc.log compile.log elaborate.log simulate.log diag_report.log xsc_report.log memory_sc.so .tmp_log xcelium_lib waves.shm c.obj)
...
Usage: memory.sh [-step]\n\
Usage: memory.sh [-lib_map_path]\n\
Usage: memory.sh [-reset_run]\n\
Usage: memory.sh [-reset_log]\n\
Usage: memory.sh [-keep_index]\n\
