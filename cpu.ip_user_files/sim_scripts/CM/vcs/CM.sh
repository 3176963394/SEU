...
#
# Script generated by Vivado on Mon May 20 17:00:34 +0800 2024
...
#
# Usage        : CM.sh
#                CM.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                CM.sh [-reset_run]
#                CM.sh [-reset_log]
#                CM.sh [-help]
#
...
#                script (type 'CM.sh -help' for more information).
#
...
# script info
echo -e "CM.sh - Script generated by export_simulation (Vivado v2023.2 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./CM.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  vcs $vcs_elab_opts xil_defaultlib.CM xil_defaultlib.glbl -o CM_simv
}

# RUN_STEP: <simulate>
simulate()
{
  ./CM_simv $vcs_sim_opts -do simulate.do
}
...
reset_run()
{
  files_to_remove=(ucli.key CM_simv vlogan.log vhdlan.log compile.log elaborate.log simulate.log .tmp_log .vlogansetup.env .vlogansetup.args .vcs_lib_lock scirocco_command.log lib_sc.so 64 AN.DB csrc CM_simv.daidir vcs_lib c.obj)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./CM.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: CM.sh [-help]\n\
Usage: CM.sh [-step]\n\
Usage: CM.sh [-lib_map_path]\n\
Usage: CM.sh [-reset_run]\n\
Usage: CM.sh [-reset_log]\n\
Usage: CM.sh [-keep_index]\n\
Usage: CM.sh [-noclean_files]\n\n\
